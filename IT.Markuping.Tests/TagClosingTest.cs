...
internal class TagClosingTest
{
    ...
    {
        var ex = Assert.Throws<ArgumentOutOfRangeException>(() => new TagClosing(-1, 0, false, false));
        Assert.That(ex.ParamName, Is.EqualTo("start"));

        ex = Assert.Throws<ArgumentOutOfRangeException>(() => new TagClosing(0, 0, false, false));
        Assert.That(ex.ParamName, Is.EqualTo("end"));

        ex = Assert.Throws<ArgumentOutOfRangeException>(() => new TagClosing(5, 4, false, false));
        Assert.That(ex.ParamName, Is.EqualTo("end"));
    }
}