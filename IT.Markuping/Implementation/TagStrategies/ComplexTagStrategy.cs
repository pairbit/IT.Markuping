...
public class ComplexTagStrategy<T> : ITagStrategy<T> where T : unmanaged, IEquatable<T>
{
    ...
    public int LtLength => _lt.Length;

    public int LtColonLength => _lt.Length + _colon.Length;

    public int LtSlashLength => _lt.Length + _slash.Length;

    public int LtSlashColonLength => _lt.Length + _slash.Length + _colon.Length;

    public ComplexTagStrategy(int length, T[] lt, T[] gt, T[] slash, T[] colon,
    ...
    public ComplexTagStrategy(MarkupAlphabet<T> abc)
    {
        _size = abc.Size;
        _lt = abc.LT.ToArray();
        _gt = abc.GT.ToArray();
    ...
    public bool IsStartOpening(ReadOnlySpan<T> data, int start, ReadOnlySpan<T> ns)
    {
        Debug.Assert(start >= 0);
    ...
    public bool IsStartOpening(ReadOnlySpan<T> data, ref int start, out TagNS ns)
    {
    ...
    public bool IsStartClosing(ReadOnlySpan<T> data, int start)
    {
        Debug.Assert(start >= 0);
    ...
    public bool IsStartClosing(ReadOnlySpan<T> data, int start, ReadOnlySpan<T> ns)
    {
        Debug.Assert(start >= 0);
    ...
    public bool IsStartClosing(ReadOnlySpan<T> data, ref int start, out TagNS ns)
    {
    ...
    public bool IsEndClosing(ReadOnlySpan<T> data, ref int end, out bool hasSpace)
    {
    ...
    public TagEnding GetEndingName(ReadOnlySpan<T> data, ref int end)
    {
        Debug.Assert(end >= 0);
    ...
    public TagEnding GetEndingAttributeStart(ReadOnlySpan<T> data, ref int end)
    {
        Debug.Assert(end >= 0);
    ...
    public TagEnding GetEndingHasAttributes(ReadOnlySpan<T> data, ref int end)
    {
